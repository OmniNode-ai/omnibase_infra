...
### Features
- feat(scripts): subscribe-topic wiring health check [OMN-7385] (#1136)
- feat: wire friction emission into build loop orchestrator [OMN-7400] (#1134)
- feat: add wire schema contracts for delegation + intelligence [OMN-7360] (#1133)
- feat: add verify-plugin-cache.sh for content-hash staleness detection [OMN-7372] (#1132)
- feat: autonomous build loop ONEX nodes [OMN-5113] (#1128)
- feat(validation): add sweep validators for stale TODOs, CLAUDE.md refs, migration conflicts (#1129)
- feat(router): intelligent model router MVP [OMN-7264] (#1125)
- feat(migration): add user_persona_snapshots table [OMN-3970] (#1124)
- feat(db): agent identity and session snapshot tables [OMN-7294] (#1120)
- feat(models): add AgentEntity and AgentBinding models for persistent agent identity [OMN-7292] (#1118)
- feat(registry): add YAML-based agent identity registry [OMN-7293] (#1119)
- feat(consumer): add agent learning extraction functions [OMN-7242] (#1110)
- feat(watchdog): add CAIA session watchdog for continuous pipeline [OMN-7288] (#1117)
- feat(consumer): add learning extraction consumer entrypoint stub [OMN-7251] (#1116)
- feat(migration): migrate 3 pilot handlers to declarative pattern [OMN-6848] (#1113)
- feat(llm): add resolution summary generator [OMN-7244] (#1109)
- feat(db): add agent_learnings table [OMN-7240] (#1107)
- feat(models): add agent learning record models for memory fabric [OMN-7241] (#1111)
- feat(store): add learning store freshness scoring utility [OMN-7243] (#1108)
- feat(delegation): wire delegation pipeline into runtime kernel [OMN-7040] (#1103)
- feat: port archive feature nodes (event-forward, llm, vector-store) [OMN-7032] (#1105)
- feat: add RSD Priority Engine four-node pattern (#1104)
- feat: add integration catalog contract YAML [OMN-7239] (#1102)
- feat(chain-learning): prompt-chain learning system with 5 ONEX nodes [OMN-7032] (#1101)
- feat: scope-check skill-to-node canary — 5 ONEX nodes with event-driven workflow [OMN-5406] (#1100)
- feat(infra): parameterize skills volume mount and remove hardcoded paths (#1099)
- feat(waitlist): Slack notification on new waitlist signup [OMN-7199] (#1091)
- feat(eval): add eval runner, baseline passthrough, metric collector, event emitter, regression check [OMN-6773, OMN-6774, OMN-6777, OMN-6779, OMN-6782] (#1090)
- feat: add GITHUB_WEBHOOK_SECRET contract for Infisical seeding [OMN-6723] (#1088)

### Bug Fixes
- fix: purge localhost fallbacks from omnibase_infra [OMN-7227] (#1127)
- fix: register ModelDispatchRoute for delegation dispatchers [OMN-7040] (#1135)
- fix(redpanda): raise topic_partitions_per_shard to 7000 [OMN-7302] (#1123)
- fix(runtime): increase healthcheck start_period and fix delegation contract [OMN-5095] (#1112)
- fix(migrations): move migrations 044-050 to docker runner path [OMN-7040] (#1106)
- fix(runtime): gate MCP handler on MCP_SERVER_ENABLED env var [OMN-7225] (#1097)
- fix(infra): use REDPANDA_ADVERTISE_HOST env var instead of hardcoded localhost [OMN-7223] (#1096)
- fix(infra): point check-stale-images.sh at generated compose [OMN-7222] (#1095)
- fix: populate introspection node_name from contract [OMN-6405] (#1092)
- fix: resolve macOS /var symlink false positive in path validation [OMN-7156] (#1093)
- fix: infra-up-runtime auto-rebuilds when code is newer than image [OMN-7214] (#1094)
- fix: populate introspection node_name from contract [OMN-6405] (#1089)
- fix: resolve macOS /var symlink false positive in path validation [OMN-7156] (#1087)
- fix(ci): auto-tag workflow matches chore: release PR titles [OMN-6909] (#1086)

### Other Changes
- test(integration): add agent identity lifecycle round-trip test [OMN-7301] (#1126)
- test(integration): agent identity lifecycle round-trip test [OMN-7301] (#1122)
- test(integration): add learning record construction test [OMN-7252] (#1115)
- test: add E2E verification script for error triage pipeline [OMN-5656] (#1114)
