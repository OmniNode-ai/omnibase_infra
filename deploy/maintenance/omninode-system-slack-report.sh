...
# in the env file against 175 keys as a positive control. The probe therefore
...
format_digest() {
  ...
  # OMN-18944. Same reasoning again, and it matters most here: the digest must
  # distinguish "every cluster has a fresh backup" from "nothing looked at the
  # backup", because those two were the SAME rendering for the entire life of
  # the backup CronJob and that is the defect this row closes.
  backup_lines=$(awk -F'|' '$1=="backup" {printf "- `%s`: %s (%s)\n", $3, $4, $2}' <<<"$snapshot")
  [[ -n "$backup_lines" ]] || backup_lines="- No backup freshness verdict this tick"
  # `next` keeps the row shapes mutually exclusive so a domain row cannot also
  # be rendered by the generic column-2 branch below it.
  issue_lines=$(awk -F'|' '($1=="ci" || $1=="runner-tree" || $1=="backup") && ($2=="WARNING" || $2=="CRITICAL") {printf "- %s `%s`: %s\n", $2, $3, $4; next} $2=="WARNING" || $2=="CRITICAL" {printf "- %s `%s`: %s %s %s\n", $2, $3, $4, $5, $6; next} $1=="WARNING" || $1=="CRITICAL" {printf "- %s `%s`: HTTP %s %s\n", $1, $2, $3, $4}' <<<"$snapshot")
