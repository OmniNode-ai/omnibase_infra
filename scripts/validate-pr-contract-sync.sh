...
while IFS= read -r file; do
    ...
    # Node handler pattern: src/omnibase_infra/nodes/<node>/handlers/*.py
    if echo "$file" | grep -qE '^src/omnibase_infra/nodes/([^/]+)/handlers/[^/]+\.py$'; then
        node="$(echo "$file" | sed -E 's|^src/omnibase_infra/nodes/([^/]+)/handlers/[^/]+\.py$|\1|')"
        touch "$tmpdir/node_handler_${node}"
        continue
    fi

    # Node contract pattern: src/omnibase_infra/nodes/<node>/contract.yaml
    if echo "$file" | grep -qE '^src/omnibase_infra/nodes/([^/]+)/contract\.yaml$'; then
        node="$(echo "$file" | sed -E 's|^src/omnibase_infra/nodes/([^/]+)/contract\.yaml$|\1|')"
