    ...
    for declarer, facts in sorted(installed.items()):
        ...
        for raw in raw_requires:
            if not isinstance(raw, str):
                continue
            # Cheap textual pre-filter: only an omni-internal-looking line is
            # worth loading a parser for, so a venv declaring none never needs
            # ``packaging`` at all.
            if not any(p in raw for p in _OMNI_INTERNAL_PREFIXES):
                continue
