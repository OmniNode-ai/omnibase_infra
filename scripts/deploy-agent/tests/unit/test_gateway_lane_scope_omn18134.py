...
class TestTheCommandsPinReachesTheGatewayScript:
    ...
    behind ``dev`` with nothing reporting it. OMN-18545 moved the caller into the
    job's ``finally``, so that particular cascade is closed; the pin this file
    asserts still has to travel, because an unpinned build is refused either way.
