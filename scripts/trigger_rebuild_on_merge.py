...
# OMN-18664: the runtime-change path classifier -- the canonical deploy-gate
# loader, LANE_STATE_PATH_PATTERNS, the segment-wise matcher and the union --
# moved to scripts/runtime_change_classifier.py so the release train can ask the
# SAME question about a commit without importing this module.
#
# It cannot import this module: this one imports click and pydantic at module
# scope and runs under `uv run`, while the train is stdlib-plus-pyyaml on bare
# python3 before any project install. Two path lists would let the train inherit
# a lab-pass receipt across a commit this trigger considers runtime-affecting,
# which is a cut on a lane proof that does not describe the code being cut.
#
# Loaded by path rather than as `scripts.runtime_change_classifier` because this
# file is executed as a script (sys.path[0] is scripts/) AND imported as a
# package module by its tests (sys.path[0] is the repo root); only a path load
# resolves identically in both. Every name is re-exported below, so this
# module's own callers and tests are unchanged by the move.
_CLASSIFIER_PATH = Path(__file__).resolve().parent / "runtime_change_classifier.py"


def _load_runtime_change_classifier_module() -> object:
    module_name = "_omnibase_infra_runtime_change_classifier"
    existing = sys.modules.get(module_name)
    if existing is not None:
        return existing
    spec = importlib.util.spec_from_file_location(module_name, _CLASSIFIER_PATH)
    if spec is None or spec.loader is None:  # pragma: no cover - import plumbing
        raise RuntimeError(f"cannot load {_CLASSIFIER_PATH}")
...
_runtime_change_classifier = _load_runtime_change_classifier_module()

#: Re-exports. These ARE the objects defined in runtime_change_classifier, not
#: copies, so a test that monkeypatches one here and a caller that reads it
#: there still see one definition.
