-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_projection_registration.
...
--   onex-prod}.yaml`) declare `node_service_registry` under
...
--   declared)`, and `node_service_registry` was among them.
--
...
--   26 -- `node_service_registry` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `0000_create_node_service_registry.sql` is the file that creates
--   it, and a grant must name the PHYSICAL schema.
--
--   `0000_create_node_service_registry.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.node_service_registry TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS node_service_registry_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'node_service_registry'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS node_service_registry_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'node_service_registry'
  AND grantee = 'omninode_runtime'
