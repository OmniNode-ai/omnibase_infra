-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_projection_sandbox_decisions.
...
--   onex-prod}.yaml`) declare `sandbox_decisions` under
...
--   declared)`, and `sandbox_decisions` was among them.
--
...
--   26 -- `sandbox_decisions` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `0001_create_sandbox_decisions.sql` is the file that creates
--   it, and a grant must name the PHYSICAL schema.
--
--   `0001_create_sandbox_decisions.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.sandbox_decisions TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS sandbox_decisions_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'sandbox_decisions'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS sandbox_decisions_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'sandbox_decisions'
  AND grantee = 'omninode_runtime'
