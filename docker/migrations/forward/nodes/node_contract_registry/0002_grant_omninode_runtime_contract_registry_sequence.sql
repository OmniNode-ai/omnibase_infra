-- OMN-17447: sequence USAGE for omninode_runtime behind contract_registry.id.
...
-- ============================================================================
--   `0000_create_contract_registry.sql` declares
--   `id SERIAL PRIMARY KEY`. PostgreSQL rewrites that into a plain
...
--   SELECT/INSERT/UPDATE grant on `contract_registry` and STILL fail
...
--   SERIAL, not IDENTITY, which is exactly the distinction that makes the
...
--    `pg_get_serial_sequence` rather than the literal `contract_registry_id_seq`,
...
--    not sequence-backed at all, which would contradict 0000_create_contract_registry.sql's
--    own SERIAL declaration -- fail loud rather than no-op into another
--    silent half-grant.
-- ---------------------------------------------------------------------------
DO $$
DECLARE
    v_seq TEXT;
BEGIN
    v_seq := pg_get_serial_sequence('public.contract_registry', 'id');
    IF v_seq IS NULL THEN
        RAISE EXCEPTION
            'OMN-17447: public.contract_registry.id is not backed by a sequence, but 0000_create_contract_registry.sql declares it SERIAL. Refusing to grant a privilege on an object that does not exist -- reconcile the column shape first.';
    END IF;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS contract_registry_id_sequence_usage_assertion
WHERE has_sequence_privilege(
          'omninode_runtime',
          pg_get_serial_sequence('public.contract_registry', 'id'),
          'USAGE'
      );
