-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_contract_registry.
...
--   onex-prod}.yaml`) carry `contract_registry` in
...
--
--   Physical location is bare `public`: 0000_create_contract_registry.sql issues an
...
--   `contract_registry` is sequence-backed, and an INSERT that omits the
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.contract_registry TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS contract_registry_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'contract_registry'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS contract_registry_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'contract_registry'
  AND grantee = 'omninode_runtime'
