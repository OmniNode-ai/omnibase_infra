-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_projection_baselines.
-- Target DB: omnidash_analytics (NODE_POSTGRES_DB)
-- Node: node_projection_baselines
--
-- ============================================================================
-- WHAT IS BROKEN
-- ============================================================================
...
--   onex-prod}.yaml`) carry `baselines_breakdown`, `baselines_comparisons`,
--   `baselines_snapshots` and `baselines_trend` in
...
--
--   Physical location is bare `public`: 0001_create_baselines_tables.sql issues an
...
--
-- NOTE (OMN-17447):
--   every relation in this file is keyed by a supplied value rather than a
--   sequence, so a TABLE grant is sufficient on its own here. OMN-17447
--   covers the sequence-backed relations elsewhere.
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.baselines_breakdown TO omninode_runtime;
GRANT SELECT, INSERT, UPDATE ON public.baselines_comparisons TO omninode_runtime;
GRANT SELECT, INSERT, UPDATE ON public.baselines_snapshots TO omninode_runtime;
GRANT SELECT, INSERT, UPDATE ON public.baselines_trend TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS baselines_breakdown_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'baselines_breakdown'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS baselines_breakdown_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'baselines_breakdown'
  AND grantee = 'omninode_runtime'
...
SELECT 1 / count(*) AS baselines_comparisons_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
...
SELECT 1 / count(*) AS baselines_comparisons_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'baselines_comparisons'
  AND grantee = 'omninode_runtime'
...
SELECT 1 / count(*) AS baselines_snapshots_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'baselines_snapshots'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS baselines_snapshots_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'baselines_snapshots'
  AND grantee = 'omninode_runtime'
...
SELECT 1 / count(*) AS baselines_trend_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'baselines_trend'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS baselines_trend_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'baselines_trend'
  AND grantee = 'omninode_runtime'
