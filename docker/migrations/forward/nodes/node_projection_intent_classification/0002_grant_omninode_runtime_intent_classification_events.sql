-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_projection_intent_classification.
...
--   onex-prod}.yaml`) carry `intent_classification_events` in
...
--
--   Physical location is bare `public`: 0000_create_intent_classification_events.sql issues an
...
--   `intent_classification_events` is sequence-backed, and an INSERT that
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.intent_classification_events TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS intent_classification_events_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'intent_classification_events'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS intent_classification_events_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'intent_classification_events'
  AND grantee = 'omninode_runtime'
