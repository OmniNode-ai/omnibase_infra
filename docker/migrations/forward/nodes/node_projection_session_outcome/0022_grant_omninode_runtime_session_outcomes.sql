-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_projection_session_outcome.
...
--   onex-prod}.yaml`) declare `session_outcomes` under
...
--   declared)`, and `session_outcomes` was among them.
--
...
--   26 -- `session_outcomes` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `0021_session_outcomes.sql` is the file that creates
--   it, and a grant must name the PHYSICAL schema.
--
--   `0021_session_outcomes.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.session_outcomes TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS session_outcomes_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'session_outcomes'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS session_outcomes_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'session_outcomes'
  AND grantee = 'omninode_runtime'
