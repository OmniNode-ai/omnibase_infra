-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_evidence_dashboard_reducer.
...
--   onex-prod}.yaml`) declare `evidence_correlation_trace_projection`, `evidence_dashboard_projection` and `evidence_readiness_aggregate_projection` under
--   `databases.application.principals.omninode_runtime.grants[object_type:
--   TABLE, schema: public]` with `[INSERT, SELECT, UPDATE]`. That list is
...
--   declared)`, and `evidence_correlation_trace_projection`, `evidence_dashboard_projection`, `evidence_readiness_aggregate_projection` were among them.
--
...
--   26 -- `evidence_correlation_trace_projection`, `evidence_dashboard_projection`, `evidence_readiness_aggregate_projection` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `0001_create_evidence_dashboard_projection_tables.sql` is the file that creates
--   them, and a grant must name the PHYSICAL schema.
--
--   `0001_create_evidence_dashboard_projection_tables.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.evidence_correlation_trace_projection TO omninode_runtime;
GRANT SELECT, INSERT, UPDATE ON public.evidence_dashboard_projection TO omninode_runtime;
GRANT SELECT, INSERT, UPDATE ON public.evidence_readiness_aggregate_projection TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS evidence_correlation_trace_projection_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'evidence_correlation_trace_projection'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS evidence_correlation_trace_projection_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'evidence_correlation_trace_projection'
  AND grantee = 'omninode_runtime'
...
SELECT 1 / count(*) AS evidence_dashboard_projection_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'evidence_dashboard_projection'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS evidence_dashboard_projection_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'evidence_dashboard_projection'
  AND grantee = 'omninode_runtime'
...
SELECT 1 / count(*) AS evidence_readiness_aggregate_projection_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'evidence_readiness_aggregate_projection'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS evidence_readiness_aggregate_projection_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'evidence_readiness_aggregate_projection'
  AND grantee = 'omninode_runtime'
