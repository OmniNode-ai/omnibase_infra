-- OMN-17447: sequence USAGE for tenant_projection_writer behind dep_health_findings.id.
-- Target DB: omnidash_analytics (NODE_POSTGRES_DB)
-- Node: node_projection_dep_health
--
-- ============================================================================
-- THE HALF A TABLE GRANT DOES NOT REACH
-- ============================================================================
--   `001_create_dep_health_findings.sql` declares
...
--   SELECT/INSERT/UPDATE grant on `dep_health_findings` and STILL fail
...
--    `pg_get_serial_sequence` rather than the literal `dep_health_findings_id_seq`,
...
--    not sequence-backed at all, which would contradict 001_create_dep_health_findings.sql's
...
DECLARE
    v_seq TEXT;
BEGIN
    v_seq := pg_get_serial_sequence('public.dep_health_findings', 'id');
    IF v_seq IS NULL THEN
        RAISE EXCEPTION
            'OMN-17447: public.dep_health_findings.id is not backed by a sequence, but 001_create_dep_health_findings.sql declares it BIGSERIAL. Refusing to grant a privilege on an object that does not exist -- reconcile the column shape first.';
    END IF;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS dep_health_findings_id_sequence_usage_assertion
WHERE has_sequence_privilege(
          'tenant_projection_writer',
          pg_get_serial_sequence('public.dep_health_findings', 'id'),
          'USAGE'
      );
