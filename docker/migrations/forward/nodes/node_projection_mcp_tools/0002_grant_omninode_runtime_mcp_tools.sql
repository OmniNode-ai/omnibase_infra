-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_projection_mcp_tools.
-- Target DB: omnidash_analytics (NODE_POSTGRES_DB)
-- Node: node_projection_mcp_tools
--
-- ============================================================================
-- WHAT IS BROKEN
-- ============================================================================
...
--   onex-prod}.yaml`) declare `mcp_tools` under
...
--   declared)`, and `mcp_tools` was among them.
--
...
--   26 -- `mcp_tools` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `0001_create_mcp_tools.sql` is the file that creates
--   it, and a grant must name the PHYSICAL schema.
--
--   `0001_create_mcp_tools.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.mcp_tools TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS mcp_tools_insert_grant_assertion
...
SELECT 1 / count(*) AS mcp_tools_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'mcp_tools'
  AND grantee = 'omninode_runtime'
