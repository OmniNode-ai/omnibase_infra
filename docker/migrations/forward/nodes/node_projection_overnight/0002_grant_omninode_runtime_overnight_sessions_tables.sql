-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_projection_overnight.
-- Target DB: omnidash_analytics (NODE_POSTGRES_DB)
-- Node: node_projection_overnight
--
-- ============================================================================
-- WHAT IS BROKEN
-- ============================================================================
...
--   onex-prod}.yaml`) carry `overnight_session_phases` and `overnight_sessions` in
...
--
--   Physical location is bare `public`: 0000_create_overnight_sessions_tables.sql issues an
...
--   `overnight_session_phases` is sequence-backed, and an INSERT that
--   omits the key fails at the SEQUENCE before it ever reaches the table.
--   That half is OMN-17447 and lands in this same lineage; neither half
--   works alone.
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.overnight_session_phases TO omninode_runtime;
GRANT SELECT, INSERT, UPDATE ON public.overnight_sessions TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS overnight_session_phases_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'overnight_session_phases'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS overnight_session_phases_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'overnight_session_phases'
  AND grantee = 'omninode_runtime'
...
SELECT 1 / count(*) AS overnight_sessions_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'overnight_sessions'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS overnight_sessions_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'overnight_sessions'
  AND grantee = 'omninode_runtime'
