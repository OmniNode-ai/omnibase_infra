-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_projection_delegation.
...
--   onex-prod}.yaml`) declare `generation_events` under
...
--   declared)`, and `generation_events` was among them.
--
...
--   26 -- `generation_events` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `0008_generation_events.sql` is the file that creates
--   it, and a grant must name the PHYSICAL schema.
--
--   `0008_generation_events.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.generation_events TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS generation_events_insert_grant_assertion
...
SELECT 1 / count(*) AS generation_events_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'generation_events'
  AND grantee = 'omninode_runtime'
