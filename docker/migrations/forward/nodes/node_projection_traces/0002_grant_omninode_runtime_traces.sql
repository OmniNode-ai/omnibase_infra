-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_projection_traces.
-- Target DB: omnidash_analytics (NODE_POSTGRES_DB)
-- Node: node_projection_traces
--
-- ============================================================================
-- WHAT IS BROKEN
-- ============================================================================
...
--   onex-prod}.yaml`) declare `traces` under
...
--   declared)`, and `traces` was among them.
--
...
--   26 -- `traces` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `0001_create_traces.sql` is the file that creates
--   it, and a grant must name the PHYSICAL schema.
--
--   `0001_create_traces.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.traces TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS traces_insert_grant_assertion
...
SELECT 1 / count(*) AS traces_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'traces'
  AND grantee = 'omninode_runtime'
