-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_projection_llm_cost.
-- Target DB: omnidash_analytics (NODE_POSTGRES_DB)
-- Node: node_projection_llm_cost
--
-- ============================================================================
-- WHAT IS BROKEN
-- ============================================================================
...
--   onex-prod}.yaml`) declare `llm_call_metrics` under
...
--   declared)`, and `llm_call_metrics` was among them.
--
...
--   26 -- `llm_call_metrics` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `0001_create_llm_call_metrics.sql` is the file that creates
--   it, and a grant must name the PHYSICAL schema.
--
--   `0001_create_llm_call_metrics.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.llm_call_metrics TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS llm_call_metrics_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'llm_call_metrics'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS llm_call_metrics_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'llm_call_metrics'
  AND grantee = 'omninode_runtime'
