-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_projection_event_chain.
...
--   onex-prod}.yaml`) declare `event_chain` under
...
--   declared)`, and `event_chain` was among them.
--
...
--   26 -- `event_chain` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `0001_create_event_chain.sql` is the file that creates
--   it, and a grant must name the PHYSICAL schema.
--
--   `0001_create_event_chain.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.event_chain TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS event_chain_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'event_chain'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS event_chain_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'event_chain'
  AND grantee = 'omninode_runtime'
