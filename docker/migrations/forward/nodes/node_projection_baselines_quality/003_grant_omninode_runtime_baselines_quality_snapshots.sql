-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_projection_baselines_quality.
-- Target DB: omnidash_analytics (NODE_POSTGRES_DB)
-- Node: node_projection_baselines_quality
--
-- ============================================================================
-- WHAT IS BROKEN
-- ============================================================================
...
--   onex-prod}.yaml`) declare `baselines_quality_snapshots` under
...
--   declared)`, and `baselines_quality_snapshots` was among them.
--
...
--   26 -- `baselines_quality_snapshots` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `001_create_baselines_quality_snapshots.sql` is the file that creates
--   it, and a grant must name the PHYSICAL schema.
--
--   `001_create_baselines_quality_snapshots.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.baselines_quality_snapshots TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS baselines_quality_snapshots_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'baselines_quality_snapshots'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS baselines_quality_snapshots_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'baselines_quality_snapshots'
  AND grantee = 'omninode_runtime'
