-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_projection_capsule_store.
-- Target DB: omnidash_analytics (NODE_POSTGRES_DB)
-- Node: node_projection_capsule_store
--
-- ============================================================================
-- WHAT IS BROKEN
-- ============================================================================
...
--   onex-prod}.yaml`) declare `capsule_store` under
...
--   declared)`, and `capsule_store` was among them.
--
...
--   26 -- `capsule_store` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `078_create_capsule_store.sql` is the file that creates
--   it, and a grant must name the PHYSICAL schema.
--
--   `078_create_capsule_store.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.capsule_store TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS capsule_store_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'capsule_store'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS capsule_store_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'capsule_store'
  AND grantee = 'omninode_runtime'
