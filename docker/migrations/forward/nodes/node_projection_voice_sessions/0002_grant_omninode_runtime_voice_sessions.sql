-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_projection_voice_sessions.
...
--   onex-prod}.yaml`) declare `voice_sessions` under
...
--   declared)`, and `voice_sessions` was among them.
--
...
--   26 -- `voice_sessions` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `0001_create_voice_sessions.sql` is the file that creates
--   it, and a grant must name the PHYSICAL schema.
--
--   `0001_create_voice_sessions.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.voice_sessions TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS voice_sessions_insert_grant_assertion
...
SELECT 1 / count(*) AS voice_sessions_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'voice_sessions'
  AND grantee = 'omninode_runtime'
