-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_deployment_evidence_reducer.
-- Target DB: omnidash_analytics (NODE_POSTGRES_DB)
-- Node: node_deployment_evidence_reducer
--
-- ============================================================================
-- WHAT IS BROKEN
-- ============================================================================
...
--   onex-prod}.yaml`) declare `deployment_evidence_projection` and `deployment_readiness_projection` under
...
--   declared)`, and `deployment_evidence_projection`, `deployment_readiness_projection` were among them.
--
...
--   26 -- `deployment_evidence_projection`, `deployment_readiness_projection` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `0001_create_deployment_evidence_projection_tables.sql` is the file that creates
--   them, and a grant must name the PHYSICAL schema.
--
--   `0001_create_deployment_evidence_projection_tables.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.deployment_evidence_projection TO omninode_runtime;
GRANT SELECT, INSERT, UPDATE ON public.deployment_readiness_projection TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS deployment_evidence_projection_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'deployment_evidence_projection'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS deployment_evidence_projection_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'deployment_evidence_projection'
  AND grantee = 'omninode_runtime'
...
SELECT 1 / count(*) AS deployment_readiness_projection_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'deployment_readiness_projection'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS deployment_readiness_projection_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'deployment_readiness_projection'
  AND grantee = 'omninode_runtime'
