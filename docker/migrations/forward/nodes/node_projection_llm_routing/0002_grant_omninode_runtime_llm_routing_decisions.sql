-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_projection_llm_routing.
-- Target DB: omnidash_analytics (NODE_POSTGRES_DB)
-- Node: node_projection_llm_routing
--
-- ============================================================================
-- WHAT IS BROKEN
-- ============================================================================
...
--   onex-prod}.yaml`) declare `llm_routing_decisions` under
...
--   declared)`, and `llm_routing_decisions` was among them.
--
...
--   26 -- `llm_routing_decisions` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `0000_create_llm_routing_decisions.sql` is the file that creates
--   it, and a grant must name the PHYSICAL schema.
--
--   `0000_create_llm_routing_decisions.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.llm_routing_decisions TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS llm_routing_decisions_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'llm_routing_decisions'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS llm_routing_decisions_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'llm_routing_decisions'
  AND grantee = 'omninode_runtime'
