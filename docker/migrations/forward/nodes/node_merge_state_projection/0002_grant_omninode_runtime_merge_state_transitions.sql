-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_merge_state_projection.
-- Target DB: omnidash_analytics (NODE_POSTGRES_DB)
-- Node: node_merge_state_projection
--
-- ============================================================================
-- WHAT IS BROKEN
-- ============================================================================
...
--   onex-prod}.yaml`) carry `merge_state_transitions` in
...
--
--   PROVEN LIVE, not inferred: `merge_state_transitions`
...
--
--   Physical location is bare `public`: 0001_create_merge_state_transitions.sql issues an
...
--   `merge_state_transitions` is sequence-backed, and an INSERT that
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.merge_state_transitions TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS merge_state_transitions_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'merge_state_transitions'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS merge_state_transitions_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'merge_state_transitions'
  AND grantee = 'omninode_runtime'
