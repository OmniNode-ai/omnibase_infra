-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_projection_cost_by_repo.
-- Target DB: omnidash_analytics (NODE_POSTGRES_DB)
-- Node: node_projection_cost_by_repo
--
-- ============================================================================
-- WHAT IS BROKEN
-- ============================================================================
...
--   onex-prod}.yaml`) declare `cost_by_repo_snapshots` under
...
--   declared)`, and `cost_by_repo_snapshots` was among them.
--
...
--   26 -- `cost_by_repo_snapshots` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `0001_create_cost_by_repo_snapshots.sql` is the file that creates
--   it, and a grant must name the PHYSICAL schema.
--
--   `0001_create_cost_by_repo_snapshots.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.cost_by_repo_snapshots TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS cost_by_repo_snapshots_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'cost_by_repo_snapshots'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS cost_by_repo_snapshots_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'cost_by_repo_snapshots'
  AND grantee = 'omninode_runtime'
