-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_nightly_loop_controller.
...
--   onex-prod}.yaml`) declare `nightly_loop_decisions` and `nightly_loop_iterations` under
...
--   declared)`, and `nightly_loop_decisions`, `nightly_loop_iterations` were among them.
--
...
--   26 -- `nightly_loop_decisions`, `nightly_loop_iterations` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `001_create_nightly_loop_tables.sql` is the file that creates
--   them, and a grant must name the PHYSICAL schema.
--
--   `001_create_nightly_loop_tables.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.nightly_loop_decisions TO omninode_runtime;
GRANT SELECT, INSERT, UPDATE ON public.nightly_loop_iterations TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS nightly_loop_decisions_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'nightly_loop_decisions'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS nightly_loop_decisions_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'nightly_loop_decisions'
  AND grantee = 'omninode_runtime'
...
SELECT 1 / count(*) AS nightly_loop_iterations_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'nightly_loop_iterations'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS nightly_loop_iterations_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'nightly_loop_iterations'
  AND grantee = 'omninode_runtime'
