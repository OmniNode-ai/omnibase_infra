-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_renderer_capability_projection.
-- Target DB: omnidash_analytics (NODE_POSTGRES_DB)
-- Node: node_renderer_capability_projection
--
-- ============================================================================
-- WHAT IS BROKEN
-- ============================================================================
...
--   onex-prod}.yaml`) declare `renderer_capability_projection` under
...
--
--   No migration in either repo has ever ISSUED it. The two halves --
...
--   declared)`, and `renderer_capability_projection` was among them.
--
...
--   26 -- `renderer_capability_projection` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `0001_create_renderer_capability_projection.sql` is the file that creates
--   it, and a grant must name the PHYSICAL schema.
--
--   `0001_create_renderer_capability_projection.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.renderer_capability_projection TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS renderer_capability_projection_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'renderer_capability_projection'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS renderer_capability_projection_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'renderer_capability_projection'
  AND grantee = 'omninode_runtime'
