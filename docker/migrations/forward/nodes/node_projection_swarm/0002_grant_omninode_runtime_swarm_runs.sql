-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_projection_swarm.
-- Target DB: omnidash_analytics (NODE_POSTGRES_DB)
-- Node: node_projection_swarm
--
-- ============================================================================
-- WHAT IS BROKEN
-- ============================================================================
...
--   onex-prod}.yaml`) declare `swarm_runs` under
...
--   declared)`, and `swarm_runs` was among them.
--
...
--   26 -- `swarm_runs` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `0001_create_swarm_runs.sql` is the file that creates
--   it, and a grant must name the PHYSICAL schema.
--
--   `0001_create_swarm_runs.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.swarm_runs TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS swarm_runs_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'swarm_runs'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS swarm_runs_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'swarm_runs'
  AND grantee = 'omninode_runtime'
