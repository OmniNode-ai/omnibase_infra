-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_pr_lifecycle_state_reducer.
...
--   onex-prod}.yaml`) carry `pr_lifecycle_ledger_entries` in
...
--
--   PROVEN LIVE, not inferred: `pr_lifecycle_ledger_entries`
...
--
--   Physical location is bare `public`: 0001_create_pr_lifecycle_ledger_entries.sql issues an
...
--   `pr_lifecycle_ledger_entries` is sequence-backed, and an INSERT that
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.pr_lifecycle_ledger_entries TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS pr_lifecycle_ledger_entries_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'pr_lifecycle_ledger_entries'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS pr_lifecycle_ledger_entries_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'pr_lifecycle_ledger_entries'
  AND grantee = 'omninode_runtime'
