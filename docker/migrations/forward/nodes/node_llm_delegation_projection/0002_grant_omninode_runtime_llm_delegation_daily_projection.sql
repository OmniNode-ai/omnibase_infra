-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_llm_delegation_projection.
...
--   onex-prod}.yaml`) declare `llm_delegation_daily_projection` under
...
--   declared)`, and `llm_delegation_daily_projection` was among them.
--
...
--   26 -- `llm_delegation_daily_projection` included -- exist ONLY because they were granted
...
--
--   Physical schema is `public`: `0001_create_llm_delegation_daily_projection.sql` is the file that creates
--   it, and a grant must name the PHYSICAL schema.
--
--   `0001_create_llm_delegation_daily_projection.sql` itself is NOT edited. It is applied on the `.201` dev
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.llm_delegation_daily_projection TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS llm_delegation_daily_projection_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'llm_delegation_daily_projection'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS llm_delegation_daily_projection_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'llm_delegation_daily_projection'
  AND grantee = 'omninode_runtime'
