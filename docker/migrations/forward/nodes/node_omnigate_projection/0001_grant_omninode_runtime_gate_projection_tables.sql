-- OMN-17440: topology-derived omninode_runtime TABLE grants for node_omnigate_projection.
...
--   onex-prod}.yaml`) carry `gate_activity` and `gate_metrics` in
...
--
--   Physical location is bare `public`: 0000_create_gate_projection_tables.sql issues an
...
--   `gate_activity` is sequence-backed, and an INSERT that omits the key
--   fails at the SEQUENCE before it ever reaches the table. That half is
--   OMN-17447 and lands in this same lineage; neither half works alone.
...
-- ---------------------------------------------------------------------------
GRANT SELECT, INSERT, UPDATE ON public.gate_activity TO omninode_runtime;
GRANT SELECT, INSERT, UPDATE ON public.gate_metrics TO omninode_runtime;
...
-- ---------------------------------------------------------------------------
SELECT 1 / count(*) AS gate_activity_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'gate_activity'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS gate_activity_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'gate_activity'
  AND grantee = 'omninode_runtime'
...
SELECT 1 / count(*) AS gate_metrics_insert_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'gate_metrics'
  AND grantee = 'omninode_runtime'
  AND privilege_type = 'INSERT';

SELECT 1 / count(*) AS gate_metrics_select_grant_assertion
FROM information_schema.role_table_grants
WHERE table_schema = 'public'
  AND table_name = 'gate_metrics'
  AND grantee = 'omninode_runtime'
