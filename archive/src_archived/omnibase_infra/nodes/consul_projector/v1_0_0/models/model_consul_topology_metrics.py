...
class ModelConsulTopologyMetrics(BaseModel):
    ...
    average_connections_per_service: float = Field(
        ..., description="Average connections per service",
    )
    cluster_coefficient: float = Field(
        ..., description="Clustering coefficient of the topology",
    )
    max_path_length: int = Field(
        ..., description="Maximum path length between services",
    )
