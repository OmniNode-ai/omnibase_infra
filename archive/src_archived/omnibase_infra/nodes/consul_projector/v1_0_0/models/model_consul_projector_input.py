...
class ModelConsulProjectorInput(BaseModel):
    ...
    projection_type: ModelConsulProjectionType = Field(
        ..., description="Type of projection to perform",
    )
    target_services: list[str] | None = Field(
        None, description="List of specific services to include in projection",
    )
    include_metadata: bool = Field(
        True, description="Whether to include metadata in projection results",
    )
