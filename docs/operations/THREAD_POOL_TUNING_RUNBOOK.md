...
**Configuration Model**: `ModelVaultAdapterConfig`
...
The VaultAdapter `health_check()` method returns thread pool metrics:
...
async def collect_metrics(adapter: VaultAdapter, service_name: str):
    health = await adapter.health_check()
...
- `"VaultAdapter initialized"` - Shows configuration values
...
The VaultAdapter uses exponential backoff with circuit breaker:
...
- [VaultAdapter Source](../../src/omnibase_infra/handlers/handler_vault.py)
- [Configuration Model](../../src/omnibase_infra/handlers/model_vault_adapter_config.py)
