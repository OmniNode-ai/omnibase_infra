...
| `QUERY_TIMEOUT_SECONDS` in transport map but not `shared_key_registry.yaml` | `QUERY_TIMEOUT_SECONDS` is a per-service setting (how long a specific service waits for a query). `POSTGRES_TIMEOUT_MS` is the shared platform key (connection-level timeout). They are different concepts at different scopes; keeping them separate is intentional. |
