...
EXPECTED_FENCE = (
    FENCED_DELEGATION_IDS + FENCED_REGISTRATION_IDS + FENCED_INFERENCE_RESPONSE_IDS
)
...
def test_manifest_pins_the_known_baseline_fence() -> None:
    ...
    reg_end = len(FENCED_DELEGATION_IDS) + len(FENCED_REGISTRATION_IDS)
    assert found[len(FENCED_DELEGATION_IDS) : reg_end] == FENCED_REGISTRATION_IDS, (
        "the OMN-15335/OMN-15343 registration hold is not the exact expected trio"
    )
    assert found[reg_end:] == FENCED_INFERENCE_RESPONSE_IDS, (
...
def test_fence_matches_omninode_infra_k8s_runner() -> None:
    ...
    expected_effective_k8s_fence = FENCED_DELEGATION_IDS + FENCED_INFERENCE_RESPONSE_IDS
    assert effective_k8s_fence == expected_effective_k8s_fence, (
        "the k8s Job's effective (post-release) fence no longer equals the "
        "delegation quartet plus the OMN-15336 item-4 inference-response hold "
