...
def _source_tree(
    ...
    *,
    ...
    if not track_source:
        _git(repository, "rm", "--cached", "-q", "src/omnibase_core/__init__.py")
    return package
...
def _run(
    ...
    return subprocess.run(
        ["/bin/bash", str(SCRIPT), "--no-color", "--path", str(path)],
        capture_output=True,
        check=False,
        env=env,
        text=True,
...
def _write_marker_decoy(fake_bin: Path, command: str, marker: Path) -> None:
    decoy = fake_bin / command
...
@pytest.mark.unit
def test_command_configuration_cannot_spoof_decoy_origin(tmp_path: Path) -> None:
    decoy = _source_tree(
        tmp_path,
...
@pytest.mark.unit
def test_git_config_object_and_worktree_selectors_cannot_spoof_candidate(
    tmp_path: Path,
) -> None:
    ...
    fake_global.write_text(f'[url "{CANONICAL_ORIGIN}"]\n\tinsteadOf = mirror:\n')
    env = _environment(
        GIT_CONFIG_NOSYSTEM="0",
        GIT_CONFIG_GLOBAL=str(fake_global),
        GIT_CONFIG_SYSTEM=str(fake_global),
        GIT_DIR=str(canonical_root / ".git"),
        GIT_WORK_TREE=str(canonical_root),
        GIT_COMMON_DIR=str(canonical_root / ".git"),
        ...
        GIT_CEILING_DIRECTORIES=str(canonical_root.parent),
        GIT_DISCOVERY_ACROSS_FILESYSTEM="1",
        GIT_NO_REPLACE_OBJECTS="1",
        GIT_REPLACE_REF_BASE=str(canonical_root / ".git" / "refs" / "replace"),
    )
...
def test_linked_worktree_discovers_canonical_sibling_source(tmp_path: Path) -> None:
    ...
    linked = tmp_path / "outside" / "infra-linked"
    linked.parent.mkdir()
...
def test_normal_checkout_discovers_canonical_sibling_core_without_explicit_path(
    ...
    """A normal sibling checkout resolves Core without caller configuration."""
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    core = _source_tree(workspace)
    infra = workspace / "omnibase_infra"
    ...
    result = _run_without_path(infra, env=env)
...
@pytest.mark.unit
@pytest.mark.parametrize(
    "command", ["env", "bash", "git", "awk", "basename", "dirname", "python3"]
)
def test_caller_path_decoys_cannot_run_during_resolution(
    ...
    fake_bin.mkdir()
    marker = tmp_path / f"{command}-invoked"
    _write_marker_decoy(fake_bin, command, marker)
...
@pytest.mark.unit
def test_ambient_python_fallback_is_absent() -> None:
    text = SCRIPT.read_text()

    assert "python3 -c" not in text
    assert "no fallback is permitted" in text
