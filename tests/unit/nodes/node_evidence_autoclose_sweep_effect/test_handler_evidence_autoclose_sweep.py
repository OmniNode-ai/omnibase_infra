...
class TestAcCoverageGapFunction:
    ...
    def test_non_probative_checks_do_not_count_toward_coverage(self):
        """The denominator is VERIFIED PROBATIVE checks, not `total_checks`.

        The OMN-17556 arithmetic exactly: four criteria, twenty-two checks, of
        which eighteen were non-probative. Under the old `total_checks`
        denominator this asked `4 > 22` and released the flip.
        """
        reason, uncovered = _ac_coverage_gap(
            "## Acceptance\n- alpha\n- beta\n- gamma\n- delta\n- epsilon\n",
            4,
        ...
        assert reason
        assert "5" in reason and "4" in reason
        assert uncovered == ("alpha", "beta", "gamma", "delta", "epsilon")

    def test_non_probative_checks_in_the_majority_are_a_gap(self):
        """The bounded-minority conjunct, on the exact OMN-17556 counters.

        Four prose criteria and four verified probative checks satisfy the
        counting bound above outright — this is the second bound, and it is the
        ...
        assert uncovered == ("alpha", "beta", "gamma", "delta")

    def test_a_bounded_minority_of_non_probative_checks_still_flips(self):
        """Positive control: the OMN-17976 counters must NOT be held.

        Four numbered criteria, four verified probative checks, two
    ...
    def test_non_probative_majority_with_no_criteria_section_is_not_a_gap(self):
        """The bounded-minority bound is ABOUT criteria, so it needs some.

        A ticket whose description declares no acceptance criteria at all was
