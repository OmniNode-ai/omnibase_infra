...
# These are the lanes that deliberately deploy the ruled six-writer set.
WRITER_REQUIRED_LANES: tuple[str, ...] = ("dev", "stability-test")
...
def _covered_projections(lane: str) -> set[str]:
    """Writer-owned projections that have a writer service on a lane."""
...
@pytest.mark.parametrize("lane", WRITER_REQUIRED_LANES)
def test_every_adopted_projection_has_a_writer_service_on_every_writer_required_lane(
    lane: str,
) -> None:
    """The ruled ADOPT set is deployed on every writer-required lane.
