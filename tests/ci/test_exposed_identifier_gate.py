...
* ``test_incident_replay_omn17288`` replays the actual pre-scrub file content out of
  this repo's own git history (no literal is committed -- it is read from objects that
  already exist), and
...
# OMN-17288 incident coordinates for THIS repo. `3f10ee5e` is the scrub merge
# (omnibase_infra#3062); its first parent is the dev tip that still carried the
# slug in all five files below. omnibase_infra never carried the tenant UUID.
INCIDENT_FIX = "3f10ee5e"
INCIDENT_FILES = (
    ...
    "tests/ci/test_omn16930_registry_resolved_conversion.py",
    "tests/fixtures/omn16906/commit-7090f386f.gh-api.json.captured",
)
...
def test_incident_replay_omn17288(tmp_path: Path) -> None:
    """Replay the five pre-scrub omnibase_infra files against the shipped denylist.
    ...
    # Every one of the five files must have produced at least one finding; a replay
    # that silently covered fewer files than the census recorded is not a replay.
