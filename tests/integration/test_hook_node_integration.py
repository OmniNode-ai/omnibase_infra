        ...
        if event is not None:
            ...
            try:
                ...
                if headers and headers.get("correlation_id"):
                    # Handle UUID objects and string representations
                    cid = headers.get("correlation_id")
                    if isinstance(cid, UUID):
                        correlation_id_value = cid
                    elif isinstance(cid, str):
                        try:
                            correlation_id_value = UUID(cid)
                        except ValueError:
                            correlation_id_value = None

                reconstructed_event = ModelOnexEvent(
                ...
                print(f"Failed to reconstruct event from message bus format: {e}")
                return False
        return True
...
class TestHookNodeIntegration:
    ...
    def container_with_mocks(self, mock_http_client, mock_event_bus):
        ...
        # Register mocked protocol implementations
    ...
    async def test_http_client_timeout_integration(self, hook_node_integration):
        ...
        # Process should handle timeout gracefully (not raise exception)
        result = await hook_node_integration.process(input_data)
    ...
    async def test_end_to_end_slack_notification_flow(self, hook_node_integration, mock_event_bus):
        ...
        slack_response = MockHttpResponse(
            status_code=200,
        ...
        # The payload is stored directly as a string dictionary (converted from json)
        request_payload = requests_made[0].payload
        assert request_payload["text"] == "🚨 Integration Test Alert"
        assert request_payload["channel"] == "#dev-alerts"
    ...
    async def test_event_bus_integration_error_scenarios(self, hook_node_integration, mock_event_bus):
        ...
        hook_node_integration._http_client.request = mock_request_network_error
        ...
        # Verify the notification failed gracefully (no exception raised)
        assert result.success is False
        assert result.notification_result.is_success is False
        assert len(result.notification_result.attempts) >= 1
