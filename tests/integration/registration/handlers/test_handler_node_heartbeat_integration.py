...
class TestHandlerNodeHeartbeatNonActiveNode:
    ...
    Note:
        Terminal states (REJECTED, LIVENESS_EXPIRED) are tested separately in
        TestHandlerNodeHeartbeatTerminalState -- OMN-4822 added a guard that
        returns no_op for terminal states, so they must NOT produce intents.
    """
...
class TestHandlerNodeHeartbeatTerminalState:
    """Tests for heartbeat handling when node is in a terminal state (OMN-4822).

    Terminal states (REJECTED, LIVENESS_EXPIRED) indicate the registration has
    ended. Heartbeats for terminal-state nodes must return no_op with zero
    intents — extending the liveness deadline would trigger spurious
    re-registration side effects.

    ONEX Contract Compliance:
        Handler returns result=None. No intents emitted for terminal states.
    """
    ...
    async def test_handle_heartbeat_for_terminal_node(
        ...
        """Verify heartbeat returns no_op for terminal-state nodes.

        OMN-4822: decide_heartbeat guards against terminal states to prevent
        spurious liveness deadline extensions.
        ...
        # OMN-4822: Terminal states must produce zero intents
        assert len(output.intents) == 0, (
            f"Expected 0 intents for terminal state={state}, got {len(output.intents)}. "
            f"node_id={node_id}, handler_id={heartbeat_handler.handler_id}"
        )
