...
"""Integration tests for AdapterProjectTrackerLinear.
...
async def test_get_issue_missing_raises_key_error() -> None:
    mcp = _FakeLinearMcp()
    adapter = AdapterProjectTrackerLinear(mcp_get_issue=mcp.get_issue)
    await adapter.connect()
...
async def test_add_comment_on_missing_issue_raises_key_error() -> None:
    mcp = _FakeLinearMcp()
    adapter = AdapterProjectTrackerLinear(mcp_add_comment=mcp.add_comment)
    await adapter.connect()
