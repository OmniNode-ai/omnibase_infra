...
"""Integration sentinel for the omnibase-spi 0.23.5 runtime protocol floor.
...
the exact floor again to the published 0.23.2 release; OMN-16997 advances it to
the published 0.23.3 release (which widens its own omnibase-core admission to
<0.48.0, unblocking the omnibase-core==0.47.1 pin here). OMN-18918 advances it
to the published 0.23.5 release, the first carrying the optional keyword-only
``delivery`` parameter on ``ProtocolDispatchEngine`` (OMN-18922), which the
runtime dispatch engine forwards source delivery coordinates through.
"""
...
def test_omnibase_spi_runtime_protocols_match_0235_floor() -> None:
    ...
    assert _minimum_for("omnibase_spi", VERSION_MATRIX) == "0.23.5"
    assert _minimum_for("omnibase_spi", _FALLBACK_MATRIX) == "0.23.5"
