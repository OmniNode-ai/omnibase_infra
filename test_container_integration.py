...
from omnibase_infra.nodes.node_postgres_adapter_effect.v1_0_0.node import NodePostgresAdapterEffect as PostgresAdapterNode
...
async def test_container_event_bus_integration():
    ...
    try:
        ...
        try:
            ...
            raise Exception(f"❌ PostgreSQL adapter injection failed: {str(e)}")
        ...
        print("\n" + "=" * 50)
        print(f"❌ Container Event Bus Integration Test FAILED")
        print(f"   Error: {str(e)}")
        return False
...
async def test_redpanda_connection_mock():
    ...
    try:
        ...
        print(f"❌ RedPanda connection test failed: {str(e)}")
        return False
