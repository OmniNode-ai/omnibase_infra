...
class ModelKafkaHealthResponse(BaseModel):
    ...
    controller_id: Optional[int] = Field(default=None, description="Current controller broker ID")
    response_time_ms: float = Field(description="Health check response time in milliseconds")
    timestamp: datetime = Field(description="Health check timestamp")
    version: Optional[str] = Field(default=None, description="Kafka version")
    ...
    lag_info: Optional[Dict[str, int]] = Field(
        default=None,
