...
class ModelKafkaProducerPoolStats(BaseModel):
    ...
    producer_stats: list[ModelKafkaProducerStats] | None = Field(
        default=None,
        description="Individual producer statistics",
    )
    topic_stats: list[ModelKafkaTopicStats] | None = Field(
        default=None,
    ...
    def determine_health_status(self) -> str:
        ...
        if self.failed_producers == 0 and self.error_rate < 1.0:
            return "healthy"
        if self.failed_producers < self.total_producers * 0.5 and self.error_rate < 5.0:
            return "degraded"
        return "unhealthy"
