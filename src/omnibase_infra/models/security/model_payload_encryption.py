...
class ModelEncryptedPayload(BaseModel):
    ...
    key_derivation_rounds: Optional[int] = Field(
        default=None,
        ge=1000,
        le=1000000,
    ...
    # Additional Context
    context_info: Optional[List[str]] = Field(
        default=None,
        max_items=10,
...
class ModelDecryptionRequest(BaseModel):
    ...
    # Security validation
    required_security_level: Optional[str] = Field(
        default=None,
        pattern="^(minimal|standard|high|maximum)$",
        description="Minimum required security level"
    )

    allowed_algorithms: Optional[List[str]] = Field(
        default=None,
        max_items=10,
