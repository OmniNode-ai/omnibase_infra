...
from omnibase_infra.models.infrastructure.postgres.model_postgres_context import (
    ModelPostgresContext,
)
from omnibase_infra.models.infrastructure.postgres.model_postgres_query_metrics import (
...
class ModelPostgresQueryResponse(BaseModel):
    ...
    data: ModelPostgresQueryResult | None = Field(
        default=None, description="Query result data",
    )
    status_message: str | None = Field(
        default=None, description="Database status message",
    )
    rows_affected: int = Field(
        default=0, description="Number of rows affected/returned",
    )
    ...
    query_metrics: ModelPostgresQueryMetrics | None = Field(
        default=None, description="Detailed query metrics",
    )
