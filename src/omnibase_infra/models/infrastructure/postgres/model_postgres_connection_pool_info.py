...
class ModelPostgresConnectionPoolInfo(BaseModel):
    ...
    total_connections: int = Field(
        description="Total number of connections in pool", ge=0,
    )
    ...
    pool_name: str | None = Field(
        default=None, description="Name of the connection pool",
    )
    average_connection_time_ms: float | None = Field(
        default=None, description="Average connection time in milliseconds", ge=0,
    )
    pool_health: str = Field(
        default="healthy",
        description="Pool health status: healthy, degraded, unhealthy",
    )
