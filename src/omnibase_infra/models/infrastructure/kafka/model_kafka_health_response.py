...
class ModelKafkaHealthResponse(BaseModel):
    ...
    broker_ids: list[int] = Field(
        default_factory=list, description="List of broker IDs",
    )
    ...
    offline_partitions: int = Field(
        default=0, description="Number of offline partitions",
    )
    controller_id: int | None = Field(
        default=None, description="Current controller broker ID",
    )
    ...
    errors: list[str] = Field(
        default_factory=list, description="Any health check errors",
    )
