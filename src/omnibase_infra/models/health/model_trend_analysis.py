...
class ModelTrendAnalysis(BaseModel):
    ...
    cpu_usage_trend: Optional[float] = Field(
        default=None,
        description="CPU usage change percentage"
    )

    memory_usage_trend: Optional[float] = Field(
        default=None,
    ...
    capacity_warning_threshold_hours: Optional[float] = Field(
        default=None,
        ge=0.0,
    ...
    # Historical context
    historical_data: Optional[List[ModelTrendDataPoint]] = Field(
        default=None,
        max_items=1000,
