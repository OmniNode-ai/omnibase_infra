...
"""Handler that collects runner health from GitHub API + SSH Docker inspection."""
...
class HandlerRunnerHealthCollector:
    """Collect runner health from GitHub Actions API and SSH Docker inspection.

    Cross-references GitHub registration state with Docker container status to
    compute ``EnumRunnerHealthState`` per runner.  Partial-source failures are
    surfaced explicitly in the returned snapshot rather than raised as exceptions.
    """
    ...
    def _classify_runner(
        ...
        """Map GitHub + Docker status signals to a single EnumRunnerHealthState."""
    ...
    async def _fetch_github_runners(self) -> tuple[list[dict[str, object]], str | None]:
        """Return (runners, error) from GitHub Actions API via asyncio subprocess."""
    ...
    async def _fetch_docker_status(
        ...
        """Return (statuses, error) from SSH Docker inspection via asyncio subprocess."""
        cmd = (
    ...
    async def _fetch_host_disk(self) -> float:
        """Return host disk usage percentage via SSH, or 0.0 on failure."""
        proc = await asyncio.create_subprocess_exec(
            "ssh",
            self._runner_host,
