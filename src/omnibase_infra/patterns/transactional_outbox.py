...
class PostgreSQLOutboxPattern:
    ...
    async def initialize_outbox_tables(self):
        ...
        try:
            ...
            raise OnexError(
                ...
                message=f"Failed to initialize outbox tables: {str(e)}"
            ) from e
    ...
    async def publish_event(self,
        ...
        try:
            ...
            raise OnexError(
                code=CoreErrorCode.DATABASE_OPERATION_ERROR,
                message=f"Failed to publish event to outbox: {str(e)}"
            ) from e
    ...
    async def _process_outbox_events(self):
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                self._logger.error(f"Error in outbox processor: {str(e)}", exc_info=True)
                await asyncio.sleep(self._poll_interval_seconds)
    ...
    async def _claim_pending_events(self) -> List[OutboxEvent]:
        ...
        try:
            async with connection_manager.transaction() as conn:
                ...
                """, datetime.now(timezone.utc) - timedelta(seconds=self._max_processing_time),
                self._batch_size)
            ...
            self._logger.error(f"Failed to claim pending events: {str(e)}")
            return []
    ...
    async def _publish_events_to_kafka(self, topic: str, events: List[OutboxEvent]):
        ...
        from ..nodes.kafka_adapter.v1_0_0.models.model_kafka_adapter_input import ModelKafkaAdapterInput
        ...
        try:
            ...
            for event in events:
                try:
                    ...
                    self._logger.error(f"Error publishing event {event.id}: {str(e)}")
            ...
            self._logger.error(f"Error in batch event publishing: {str(e)}")
    ...
    async def _update_event_statuses(self, events: List[OutboxEvent]):
        ...
        try:
            ...
            self._logger.error(f"Failed to update event statuses: {str(e)}")
    ...
    async def get_outbox_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self._logger.error(f"Failed to get outbox statistics: {str(e)}")
            return {"error": str(e)}


# Global outbox pattern instance
_outbox_pattern: Optional[PostgreSQLOutboxPattern] = None
