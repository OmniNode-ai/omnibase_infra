...
class InfraTimeoutError(RuntimeHostError):
    ...
    Typing Requirements:
        Context is REQUIRED for timeout errors. Use ModelTimeoutErrorContext
        (preferred) for stricter typing with required correlation_id, or
        ModelInfraErrorContext for backwards compatibility.
    ...
    Example (Preferred - ModelTimeoutErrorContext):
    ...
    Example (Legacy - ModelInfraErrorContext):
        >>> context = ModelInfraErrorContext(
        ...     transport_type=EnumInfraTransportType.DATABASE,
        ...     operation="execute_query",
        ...     correlation_id=uuid4(),  # Recommended to include
        ... )
        >>> raise InfraTimeoutError(
        ...     "Database query exceeded timeout",
        ...     context=context,
        ...     timeout_seconds=30,  # Passed as extra_context
        ... )
    """
    ...
    def __init__(
        self,
        message: str,
        context: ModelTimeoutErrorContext | ModelInfraErrorContext,
        **extra_context: object,
    ) -> None:
        """Initialize InfraTimeoutError with required context.

        Args:
            message: Human-readable error message
            context: Required infrastructure context. Use ModelTimeoutErrorContext
                for stricter typing (correlation_id guaranteed), or
                ModelInfraErrorContext for backwards compatibility.
            **extra_context: Additional context information (e.g., timeout_seconds)

        Note:
            Context is required (not optional) to enforce proper error tracking.
            The correlation_id is auto-generated by RuntimeHostError if not
            present in context, but using ModelTimeoutErrorContext ensures
            this at type-check time rather than runtime.
        """
        ...
        if isinstance(context, ModelTimeoutErrorContext):
