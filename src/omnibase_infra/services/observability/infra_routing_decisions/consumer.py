...
Example:
    ...
    ... )
    >>> consumer = InfraRoutingDecisionsConsumer(config)
    >>> await consumer.start()
    >>> await consumer.run()

    # Or run as module:
    # python -m omnibase_infra.services.observability.infra_routing_decisions.consumer
"""
...
class ConsumerMetrics:
    """Metrics tracking for the infra routing decisions consumer."""
...
class InfraRoutingDecisionsConsumer:
    """Async Kafka consumer for infra routing decision events.

    Subscribes to the routing-decided topic and persists events to PostgreSQL
    via WriterInfraRoutingDecisionsPostgres.
    ...
    def __init__(self, config: ConfigInfraRoutingDecisionsConsumer) -> None:
        self.config = config
        ...
        self._writer: WriterInfraRoutingDecisionsPostgres | None = None
        self._running = False
    ...
    async def start(self) -> None:
        ...
        logger.info(
            "Starting InfraRoutingDecisionsConsumer",
            extra={
        ...
        self._writer = WriterInfraRoutingDecisionsPostgres(
            pool=self._pool,
        ...
        self._running = True
        logger.info("InfraRoutingDecisionsConsumer started successfully")

    async def stop(self) -> None:
        """Gracefully stop consumer, producer, and pool."""
        logger.info("Stopping InfraRoutingDecisionsConsumer")
        self._running = False
    ...
    async def _process_batch(
        ...
        if events:
            try:
                count = await self._writer.write_routing_decisions(events)
                await self.metrics.record_processed(count=count)
            except Exception:
                logger.exception("Failed to write infra routing decisions batch")
                await self.metrics.record_failed(count=len(events))
                for record in batch:
    ...
    async def run(self) -> None:
        ...
        logger.info(
            "InfraRoutingDecisionsConsumer entering run loop",
            extra={
                "batch_size": self.config.batch_size,
        ...
        logger.info("InfraRoutingDecisionsConsumer run loop exited")
...
async def _main() -> None:
    ...
    active_consumer: list[InfraRoutingDecisionsConsumer | None] = [None]
    ...
    await run_with_restart(
        _run_once,
        name="InfraRoutingDecisionsConsumer",
