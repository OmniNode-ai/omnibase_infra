...
"""Async Kafka Consumer for Context Integrity Audit Observability (OMN-5240).

Consumes context audit events from Kafka topics and persists them to
PostgreSQL via WriterContextAuditPostgres.
...
Topics consumed (OMN-5234: context integrity audit event topics):
...
Related Tickets:
    ...
    - OMN-2934: Skill lifecycle consumer (reference pattern)
...
class ConsumerMetrics:
    """Metrics tracking for the context audit consumer."""
...
class ContextAuditConsumer:
    ...
    via WriterContextAuditPostgres.
    ...
    def __init__(self, config: ConfigContextAuditConsumer) -> None:
        ...
        self._writer: WriterContextAuditPostgres | None = None
        self._running = False
    ...
    async def start(self) -> None:
        ...
        # Writer
        self._writer = WriterContextAuditPostgres(
            pool=self._pool,
    ...
    async def stop(self) -> None:
        ...
        logger.info("Stopping ContextAuditConsumer")
        self._running = False
        ...
        logger.info("ContextAuditConsumer stopped")
    ...
    async def _process_batch(
        ...
        All audit events go into a single list regardless of topic — the table
        schema captures event_type so no routing is needed.
        ...
        if audit_events:
            try:
                count = await self._writer.write_audit_events(audit_events)
                await self.metrics.record_processed(count=count)
            except Exception:
                logger.exception("Failed to write context audit events batch")
                await self.metrics.record_failed(count=len(audit_events))
    ...
    async def run(self) -> None:
        ...
        logger.info(
            "ContextAuditConsumer entering run loop",
            extra={
                "batch_size": self.config.batch_size,
        ...
        logger.info("ContextAuditConsumer run loop exited")
    ...
    def _build_health_response(self) -> tuple[dict[str, object], int]:
        ...
        Idle-aware health: When the consumer is running and polling Kafka but
        has no incoming traffic (messages_received == 0), it is considered idle
        — not unhealthy. Write staleness only applies when the consumer has
        received messages that should have produced writes.
