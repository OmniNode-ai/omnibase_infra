...
class ModelDogfoodDelegationFaultRoute(BaseModel):
    ...
    # The overlay key is ``backend_id`` (the omnimarket lane declaration and the
    # delegation wire both spell it so). It is a human-readable route slug, not
    # an entity reference, so the Python attribute is named for what it holds.
    backend_key: str = Field(min_length=1, alias="backend_id")
...
def resolve_dogfood_delegation_fault_route(
    *,
    ...
    """
    runtime_environment = environment.strip()
    broker = bootstrap_servers.strip()
    if not runtime_environment or not broker:
        raise InfraUnavailableError(
    ...
    lanes = raw.get("lanes") if isinstance(raw, dict) else None
    dogfood = lanes.get(_DOGFOOD_LANE) if isinstance(lanes, dict) else None
    if not isinstance(lanes, dict) or not isinstance(dogfood, dict):
        raise InfraUnavailableError("dogfood lane declaration is absent")
    # Classify by the bounded route gate's own rule (OMN-18933): the shared
    # compose listener redpanda:9092 alone never claims dogfood, so a pin on
    # any other compose lane that reports that listener is refused here.
    claimed = claimed_bounded_lane(
        environment=runtime_environment, broker=broker, lanes=lanes
    )
    if claimed != _DOGFOOD_LANE:
        raise InfraUnavailableError(
            "dogfood fault backend pins require dogfood lane and broker identity"
        )
    validate_bounded_lane_broker_identity(
    ...
    for route in load_dogfood_delegation_fault_routes(path_for_test=path_for_test):
        if route.backend_key == backend_id:
            return route
