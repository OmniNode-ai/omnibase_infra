...
class OverlayWriter:
    ...
    def write(self, overlay: ModelOverlayFile, target_path: Path) -> None:
        ...
        # overlay_version may serialize as a dict (ModelSemVer) — normalize to string
        # so the YAML file round-trips through OverlayFileLoader cleanly.
        if isinstance(data.get("overlay_version"), dict):
            data["overlay_version"] = str(overlay.overlay_version)
