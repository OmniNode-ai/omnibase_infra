...
"""Evaluator for validating invariants against execution outputs.

The InvariantEvaluator class for validating
that outputs conform to defined invariants (validation rules).

Thread Safety:
    InvariantEvaluator is NOT thread-safe. Create separate instances
...
Example:
    >>> from omnibase_infra.nodes.node_invariant_evaluate_compute import (
    ...     InvariantEvaluator,
    ... )
...
@allow_any("OMN-11548 migrated legacy invariant evaluator uses dynamic JSON values")
class InvariantEvaluator:
    ...
    def _evaluate_threshold(
        ...
        if min_value is not None:
            try:
                ...
                min_num = float(min_value)  # type: ignore[arg-type]  # substrate-allow: OMN-11548 migrated invariant numeric config coercion
        ...
        if max_value is not None:
            try:
                # NOTE(OMN-1302): Config value from dict lookup. Safe because ValueError caught below.
                max_num = float(max_value)  # type: ignore[arg-type]  # substrate-allow: OMN-11548 migrated invariant numeric config coercion
    ...
    def _evaluate_latency(
        ...
        try:
            # NOTE(OMN-1302): Config value from dict lookup. Safe because ValueError caught below.
            max_ms_num = float(max_ms)  # type: ignore[arg-type]  # substrate-allow: OMN-11548 migrated invariant numeric config coercion
    ...
    def _evaluate_cost(
        ...
        try:
            # NOTE(OMN-1302): Config value from dict lookup. Safe because ValueError caught below.
            max_cost_num = float(max_cost)  # type: ignore[arg-type]  # substrate-allow: OMN-11548 migrated invariant numeric config coercion
        except VALIDATION_ERRORS:  # fallback-ok: invalid config fails validation
        ...
        if found:
            ...
            if found:
                try:
                    ...
                    # NOTE(OMN-1302): Config value from dict lookup. Safe because ValueError caught below.
                    actual_cost = token_count * float(cost_per_token)  # type: ignore[arg-type]  # substrate-allow: OMN-11548 migrated invariant numeric config coercion
                except (
                    TypeError,
                    ValueError,
