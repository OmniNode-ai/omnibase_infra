...
"""Dispatcher adapter for inference response events.

Routes ModelInferenceResponseData payloads to
HandlerDelegationWorkflow.handle_inference_response(), which transitions
ROUTED -> INFERENCE_COMPLETED and emits ModelQualityGateIntent.
"""
...
__all__ = ["DispatcherInferenceResponse"]
...
class DispatcherInferenceResponse(MixinAsyncCircuitBreaker):
    """Dispatcher for inference response events."""
    ...
    def __init__(
        ...
        self._init_circuit_breaker(
            threshold=3,
            reset_timeout=20.0,
            service_name="dispatcher.delegation.inference-response",
            transport_type=EnumInfraTransportType.KAFKA,
        )

    @property
    def dispatcher_id(self) -> str:
        return "dispatcher.delegation.inference-response"
    ...
    def message_types(self) -> set[str]:
        return {"ModelInferenceResponseData", "omnibase-infra.inference-response"}
    ...
    async def handle(
        ...
        try:
            ...
            payload = raw_payload
            if not isinstance(payload, ModelInferenceResponseData):
                if isinstance(payload, dict):
                    payload = ModelInferenceResponseData.model_validate(payload)
                else:
                    return ModelDispatchResult(
                        dispatch_id=uuid4(),
                        status=EnumDispatchStatus.INVALID_MESSAGE,
                        topic=TOPIC_ID_INFERENCE_RESPONSE,
                        dispatcher_id=self.dispatcher_id,
                        started_at=started_at,
                        completed_at=started_at,
                        duration_ms=0.0,
                        error_message=f"Expected ModelInferenceResponseData, got {type(payload).__name__}",
            ...
            events = self._handler.handle_inference_response(payload)
            ...
            logger.info(
                "DispatcherInferenceResponse processed inference response",
                extra={
