...
# TypeVar for generic handler resolution (unbound for Protocol compatibility)
...
class NodeRegistryEffect(MixinAsyncCircuitBreaker):
    ...
    Dependency Injection:
        ...
        **Prerequisites**:
        ...
        - ProtocolConsulExecutor with name="consul" (Consul handler)
        - ProtocolDbExecutor with name="postgres" (PostgreSQL handler)
    ...
    def __init__(
        ...
        Args:
            container: ONEX container with registered handler services.
                Must have ProtocolConsulExecutor with name="consul" and
                ProtocolDbExecutor with name="postgres" registered.
    ...
    async def _resolve_dependencies(self) -> None:
        ...
        self._consul_handler = await self._resolve_required_handler(
            protocol_type=ProtocolConsulExecutor,
            handler_name="consul",
        ...
        self._db_handler = await self._resolve_required_handler(
            protocol_type=ProtocolDbExecutor,
    ...
    async def _resolve_required_handler(
        ...
        Args:
            protocol_type: The protocol type to resolve (ProtocolConsulExecutor or ProtocolDbExecutor).
    ...
    @property
    def consul_handler(self) -> ProtocolConsulExecutor:
    ...
    @property
    def db_handler(self) -> ProtocolDbExecutor:
    ...
    async def initialize(self) -> None:
        ...
        Raises:
            RuntimeError: If dependency resolution fails (missing required handlers).
                Required handlers: consul (ProtocolConsulExecutor),
                postgres (ProtocolDbExecutor).
