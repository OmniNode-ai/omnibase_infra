...
"""Dispatcher adapter for HandlerNodeHeartbeat.

This module provides a ProtocolMessageDispatcher adapter that wraps
HandlerNodeHeartbeat for integration with MessageDispatchEngine.

The adapter:
- Deserializes ModelEventEnvelope payload to ModelNodeHeartbeatEvent
...
TOPIC_ID_NODE_HEARTBEAT = "node.heartbeat"


class DispatcherNodeHeartbeat(MixinAsyncCircuitBreaker):
    """Dispatcher adapter for HandlerNodeHeartbeat.

    This dispatcher wraps HandlerNodeHeartbeat to integrate it with
    MessageDispatchEngine's category-based routing. It handles:

    - Deserialization: Validates and casts payload to ModelNodeHeartbeatEvent
    ...
    Circuit Breaker Configuration:
        ...
        - service_name: dispatcher.registration.node-heartbeat
    ...
    def dispatcher_id(self) -> str:
        ...
        """
        return "dispatcher.registration.node-heartbeat"
    ...
    def category(self) -> EnumMessageCategory:
        ...
        Returns:
            EnumMessageCategory: EVENT category (heartbeat events).
        """
        ...
        try:
            ...
            if not isinstance(payload, ModelNodeHeartbeatEvent):
                ...
                if isinstance(payload, dict):
                    payload = ModelNodeHeartbeatEvent.model_validate(payload)
                else:
                    ...
                    return ModelDispatchResult(
                        ...
                        topic=TOPIC_ID_NODE_HEARTBEAT,
                        dispatcher_id=self.dispatcher_id,
                        started_at=started_at,
                        completed_at=started_at,
                        duration_ms=0.0,
                        error_message=f"Expected ModelNodeHeartbeatEvent payload, "
            ...
            handler_envelope: ModelEventEnvelope[ModelNodeHeartbeatEvent] = (
                ModelEventEnvelope(
                    envelope_id=uuid4(),
                    payload=payload,
            ...
            logger.info(
                "DispatcherNodeHeartbeat processed event",
                extra={
            ...
            logger.warning(
                "DispatcherNodeHeartbeat received invalid message: %s",
                sanitized_error,
                extra={
                    "duration_ms": duration_ms,
                    "correlation_id": str(correlation_id),
            ...
            logger.error(
                "DispatcherNodeHeartbeat failed: %s",
                sanitized_error,
                extra={
                    "duration_ms": duration_ms,
                    "correlation_id": str(correlation_id),
