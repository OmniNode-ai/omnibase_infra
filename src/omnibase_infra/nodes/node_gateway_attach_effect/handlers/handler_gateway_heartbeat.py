...
This is the *continuous* revocation-detection path: disabling the tenant's
Keycloak client makes ``introspect_token`` (RFC 7662) return ``active:
false`` on the next heartbeat, which flips the session to REVOKED and
deletes it from the store -- independent of the presented token's own
unexpired ``exp`` claim.

It is no longer the *only* revocation-detection path. OMN-16032 moved
introspection into ``_keycloak_introspection.introspect_token`` and made
``HandlerGatewayAttach`` call it too, so a disabled client is refused when it
tries to open a session rather than ~one heartbeat interval afterwards. This
handler's behaviour is unchanged by that move; it now shares the call instead
of owning it.

The JWKS fetch (OMN-15918 R1) stays inline in this module, and the shared
introspection call lives in a sibling module under the same ``handlers/``
package, because the imperative-contract-guard requires raw-transport calls
to live under ``handlers/``, never in a freestanding module the guard cannot
attribute to a contract-declared handler.
