"""
IntentPublisherMixin - Coordination I/O capability for ONEX nodes.
...
Usage:
    class MyReducer(IntentPublisherMixin):
...
Dependencies:
    ...
    - OnexEnvelopeV1 (from omnibase_core)

TODO: Copy to omnibase_core/mixins/ for framework-wide availability
"""
...
class IntentPublisherMixin:
    ...
    Example:
        class MetricsReducer(IntentPublisherMixin):
            def __init__(self, container):
                self._init_intent_publisher(container)
    ...
    def _init_intent_publisher(self, container: Any) -> None:
        ...
        if self._intent_kafka_client is None:
            raise ValueError(
                "IntentPublisherMixin requires 'kafka_client' service in container. "
    ...
    async def publish_event_intent(
        ...
        Returns:
            IntentPublishResult with intent_id and metadata
        ...
        if not hasattr(event, "model_dump"):
            raise AttributeError(
                f"Event must be a Pydantic model with model_dump() method. "
        ...
        # Wrap in OnexEnvelopeV1 for standard event format
        try:
            from omnibase_core.models.core import OnexEnvelopeV1
            ...
            # Fallback if OnexEnvelopeV1 not available (should not happen)
